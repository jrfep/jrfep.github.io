...
aIDs <- album_info %>% filter(title %in% c("Lugareños", "Eventuales")) %>% pull(id)
...
for (i in seq(along=photos$mediaItems.id)[photos$album %in% "Lugareños"]) {
  photo <- photos %>% slice(i)
  durl <- sprintf("%s=w400-h400-d", photo$mediaItems.baseUrl)
  dfile <- sprintf("%s/%s",img_folder, photo$mediaItems.filename)
...
if (!dir.exists(img_folder))
  dir.create(img_folder)
for (i in seq(along=photos$mediaItems.id)[photos$album %in% "Eventuales"]) {
  photo <- photos %>% slice(i)
  durl <- sprintf("%s=w400-h400-d", photo$mediaItems.baseUrl)
  dfile <- sprintf("%s/%s",img_folder, photo$mediaItems.filename)
  if (!file.exists(dfile))
